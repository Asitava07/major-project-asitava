...
<div class="login">
    ...
    <div class="login_section_2">
        <div class="login_section_2_login">
            <strong>
                <p>Welcome again to Learning Bee!</p>
            </strong>
            <p style="font-weight:800;color:darkred">Log in!</p>
            <div class="login_option">
                <div id="bt" class="login_option_overlay"></div>
                <button onclick="user()">User</button>
                <button onclick="admin()">Admin</button>
            </div>
            <form action="/login" id="user" class="login_form" method="post">
                @if(Session::has('fail'))
                <div class="alert-loginerror">
                    {{Session::get('fail')}}
                </div>
                @endif
                {{ csrf_field() }}
                ...
                <div class="login_form_submenu">
                    <div class="rememberMe">
                        <input type="checkbox" />
                        <p>Remember Me</p>
                    </div>
                    <a href="#" style="font-weight:800;color:darkblue;text-decoration:none">
                        <p>Forgot Password?</p>
                    </a>
                </div>
                <button type="submit">Login</button>
                <div class="have-account">
                    <p>Don't have any account?<a href="/register"> Create Now</a></p>
                </div>
            </form>
            <form id="admin" class="login_form" action="/adminlogin" method="post">
                @if(Session::has('fail'))
                <div class="alert-loginerror">
                    {{Session::get('fail')}}
                </div>
